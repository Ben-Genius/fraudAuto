...
const Header = () => {
  ...
  const getNavLinkClass = (path: string) => {
    ...
    if (isScrolled) {
      return `${baseClass} ${isCurrentPage
  ...
  return (
    <header
      className={`fixed top-0 z-50 w-full transition-all duration-300 border-0 ${isScrolled
    ...
    >
      ...
      >
        <div className="flex justify-between items-center border-0 ">
          ...
          <Link to="/">
            <Logo
              textClassName={`text-lg font-bold transition-colors ${isScrolled ? "text-gray-900" : "text-primary-red"
                }`}
            />
          </Link>
          ...
          <div className="hidden lg:flex items-center space-x-2 lg:space-x-3">
            <Link
              to="/login"
              className={`text-sm transition-colors px-3 lg:px-4 py-2 rounded-md ${isActive("/login")
                ? "text-primary-red font-semibold"
                : isScrolled
                  ? "text-gray-600 hover:text-gray-800"
            ...
            <Link
              to="/register"
              className={`text-sm px-3 lg:px-4 py-2 rounded-md transition-colors ${isActive("/register")
        ...
        {isMenuOpen && (
          <div
            className={`lg:hidden mt-4 pt-4 ${isScrolled
          ...
          >
            <div className="flex flex-col space-y-3">
              ...
              <div
                className={`flex space-x-3 pt-3 ${isScrolled
                  ? "border-t border-gray-200/60"
              ...
              >
                <Link
                  to="/login"
                  className={`text-sm transition-colors ${isActive("/login")
                    ? "text-primary-red font-semibold"
                    : isScrolled
                ...
                <Link
                  to="/register"
                  className={`text-sm px-4 py-2 rounded-md transition-colors ${isActive("/register")
                    ? "bg-primary-red text-white font-semibold"
                    : isScrolled
                      ? "bg-secondary-orange hover:bg-primary-yellow text-white"
